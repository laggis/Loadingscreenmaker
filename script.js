...
For support or questions, visit our Discord server: ${document.getElementById('discordLink').value}`;
}
...
function generateClientLua() {
    return `local function ShutdownLoadingScreen()
...
function generateLoadingScreenHTML() {
    ...
    const video = document.getElementById('background-video').files[0];
    let videoElement = '';
    if (video) {
        videoElement = '<video autoplay loop muted style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; z-index: -1;"><source src="background-video.mp4" type="video/mp4"></video>';
    }
